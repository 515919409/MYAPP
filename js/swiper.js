 ...
 * 选项
 ...
 new Swiper(container);// loop : truen, auto: true, time: 2000,  pagination: true
 */
//函数声明
function Swiper(selector, options) {
    ...
    swiperContainer.addEventListener('touchstart', function (e) {
        ...
        isFirst = true;
        //执行用户的自定义代码
        if(options && options.callback && typeof options.callback.start === 'function'){
    ...
    // this.name = 'iloveyou';   this.index   swiperContainer.index
    this.getIndex = function(){
        return index;
    }

    // swiperContainer.index = index;// 0  {a:100}  => {a:200}   a =100 b=a  a=200 b=?
